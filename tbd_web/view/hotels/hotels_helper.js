	...
	jQuery.ajax({
		...
		success: function (response) {
			...
			// Main logic for handling API responses based on state
				...
				console.log(response);
				allCityCodes = response.object;
				try {
					if (allCityCodes && Array.isArray(allCityCodes) && allCityCodes.length > 0) {
						localStorage.setItem("tbd_hotel_city_codes", JSON.stringify(allCityCodes));
					}
				} catch (e) {}
				hideLoader();
				//hotelCityPicker(response.CityList, element);
			} else if (state == "getHotelCodesFromDB") {
				console.log(response);
				if (response.responseCode == 200) {
					jQuery("#locationBox").data("hotelCodes", response.object);
					// Cache only default city (144306) response; element is "hotelCodesDefault" for that call only
					if (element === "hotelCodesDefault" && response.object && Array.isArray(response.object) && response.object.length > 0) {
						try {
							localStorage.setItem("tbd_hotel_codes_default_city", JSON.stringify(response.object));
						} catch (e) {}
					}
				...
				class Session {
					constructor() {
						...
						this.isActive = true;
						this.expiryTime = 38 * 60 * 1000; // 38 minutes in milliseconds
					}
				...
				console.log(response);
				if (response.object.Error.ErrorCode == 1) {
					// Show that there are no hotels available for the given location via an inbuilt dialog
			...
			else if (state == "preBookHotel") {
				console.log(response);
				hideLoader();
				if (response.object.Status.Code == 200) {
					console.log(response);
					console.log(element);
					if (response.response == 'getOrderId') {
						console.log(response.object);
						// Call the Razorpay window directly
						...
						var options = {
							...
							currency: "INR",
							name: "Travel Buddy - Hotel Booking",
							order_id: response.object.hotelPaymentResponse.id, //Order ID
							description: "Hotel Booking -" + response.object.hotelPaymentResponse.id,
							handler: function (response) {
								console.log(response);
								// Call the bookHotel API with the response
								jsInit('bookHotel', element);
							},
							modal: { escape: true }
						};
				...
				// Hide room selection page before navigating
				jQuery('#roomSelectionPage').addClass('hidden');
					showPassengerDetailsPage(response);
				} else {
					// On error, go back to room selection so user can retry
					jQuery('#roomSelectionPage').removeClass('hidden');
					alert('Pre-booking failed. Please try selecting a different room.');
				}
				//renderHotelsSearchResults(response);
			}
			else if (state == "bookHotel") {
				console.log(response);
				hideLoader();
				handleHotelBookingSuccess(response.object);
			}
			...
			} else if (state == "sendOTPDomestic") {
				if (response.success) {
					//toast('OTP Sent to your number');
					jQuery('#hotelLoginOtpSection').show();
					jQuery('#hotelLoginSendOtp')
						.prop('disabled', false)
						.text('Send OTP');
				} else {
			...
			} else if (state == "hotelVerifyAndLogin") {
				if (response.responseCode == 200) {
					tokenMaster("set", response.token);
					guestMaster("clean");
					localStorage.removeItem('isGuestUser');
					closeHotelLoginModal();
					//toast('Logged in successfully!');
					jQuery('.login-promo').addClass('hidden');
					jQuery('.coupon-hint').html(
						'<i class="fas fa-gift"></i> TB Gift Cards can be applied at payment step',
					);
				} else {
					//toast(response.errorMessage || 'Verification failed');
					jQuery('#hotelLoginVerifyBtn')
						.prop('disabled', false)
						.text('Verify & Login');
				}
